...
function discKeyFromLabel(label){
  ...
  // Para
  if(s.includes("para")){
    if(s.includes("18") || s.includes("salle")) return "para18";
    return "paraext";
  }
...
const DISC_STYLE = {
  tae:      { fill:"#f6ff00", stroke:"#202020", text:"#000000" },
  "18m":    { fill:"#f59e0b", stroke:"#6b3f00", text:"#111827" },
  paraext:  { fill:"#2563eb", stroke:"#0b2a73", text:"#ffffff" }, // bleu roi
  para18:   { fill:"#0b1f4b", stroke:"#071634", text:"#ffffff" }, // bleu marine
  "3d":     { fill:"#8b5a2b", stroke:"#4a2f15", text:"#ffffff" },
  nature:   { fill:"#556b2f", stroke:"#2e3a19", text:"#ffffff" },
  beursault:{ fill:"#ffffff", stroke:"#000000", text:"#000000", bi:true },
  campagne: { fill:"#000000", stroke:"#ffd400", text:"#ffd400", bi:true },
  jeune:    { fill:"#7c3aed", stroke:"#3b1a7b", text:"#ffffff" },
  loisir:   { fill:"#e11d48", stroke:"#7a0c22", text:"#ffffff" } // fuchsia
};
...
async function loadConcours26(){
  ...
  concoursList = rows.map((r, idx)=>{
    ...
    // mandat : colonne Mandat OU premier lien http
    ...
    return { uid, title, disc, dept, start, end, city, cp, lieu, lat, lon, mandat, code_structure };
  });
...
function renderMarquee(){
  ...
  const now = today00();
  const list = getFilteredConcours().filter(c=>{
    if(!c.start) return false;
    const d = new Date(c.start.getFullYear(), c.start.getMonth(), c.start.getDate());
  ...
  track.innerHTML = list.map(c=>{
    const when = fmtDateFR(c.start);
    return `<span class="chip">${safeText(when)} • ${safeText(c.title)}</span>`;
  }).join("");
}
...
function renderTimeline(){
  ...
  items.forEach(d=>{
    ...
    const dates = `${fmtDateFR(d.start)}${(d.end && isoDate(d.end)!==isoDate(d.start))?(" → "+fmtDateFR(d.end)):""}`;
    line.innerHTML = `
      <div class="tl-date">${safeText(dates)}</div>
      <div class="tl-body">
        <div class="tl-title">${safeText(d.title)}</div>
        <div class="tl-meta">${safeText(d.type)}${d.lieu?(" • "+safeText(d.lieu)):""}${d.precision?(" • "+safeText(d.precision)):""}</div>
        ${d.mandat?`<div class="tl-actions"><a href="${safeText(d.mandat)}" target="_blank" rel="noopener">📄 Document</a></div>`:""}
      </div>
    `;
    tl.appendChild(line);
  });
